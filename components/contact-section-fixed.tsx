...
export function ContactSection() {
  ...
  const submitToFirebase = async (data: FormData) => {
    try {
      ...
      await addDoc(collection(db, "devis-requests"), {
        ...
        status: "nouveau",
        source: "site-web",
        ip: "unknown", // Vous pouvez ajouter l'IP si nécessaire
      })
  ...
  const submitToEmailService = async (data: FormData) => {
    // Alternative : envoyer par email via un service comme EmailJS ou Formspree
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Tentative 1: Firebase
      try {
        await submitToFirebase(formState)
        console.log("✅ Envoyé via Firebase")
      } catch (firebaseError: any) {
        console.log("❌ Firebase échoué, tentative alternative...")

        // Tentative 2: API Route (si disponible)
        try {
          await submitToEmailService(formState)
          console.log("✅ Envoyé via API")
        } catch (apiError) {
          console.log("❌ API échouée, sauvegarde locale...")

          // Tentative 3: Sauvegarde locale + log
          ...
          const existingSubmissions = JSON.parse(localStorage.getItem("webzy-submissions") || "[]")
          existingSubmissions.push(submissionData)
          localStorage.setItem("webzy-submissions", JSON.stringify(existingSubmissions))

          console.log("📝 Données sauvegardées localement:", submissionData)

          // Vous pouvez aussi envoyer les données vers un webhook externe
          try {
            ...
            console.log("Webhook non disponible")
          }
        }
      }

      // Succès dans tous les cas pour l'utilisateur
      setIsSubmitted(true)
  ...
  return (
    <section id="contact" className="py-24 bg-gradient-to-b from-black to-[#0A0A20] relative overflow-hidden">
      {/* Background elements */}
